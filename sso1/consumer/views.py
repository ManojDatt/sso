...
class LogoutView(View):
    ...
    def get(self, request):
        host = urljoin(self.client.server_url, 'logout/?redirect_url=http://localhost:9001')
...
class Client:
    ...
    def get_urls(self):
        return [
            re_path(r'^login/$', self.login_view.as_view(client=self), name='sso-login'),
            re_path(r'^logout/$', self.logout_view.as_view(client=self), name='sso-logout'),
            re_path(r'^authenticate/$', self.authenticate_view.as_view(client=self), name='sso-authenticate'),
        ]